...
- **リッチテキスト描画**（Issue #13）: Workflowyはインライン装飾をノード名/noteのHTMLタグとして
  ...
  `<a>`も画像化。一覧では高さ72px、詳細シートでは240pxにCSSで制限）。詳細シートと
  登録ノードビュー/サブツリードリルダウンの行では、画像の下に元URLを小さなリンクで
  併記する（`imageUrls`オプション。画像がURLを隠したままだと値の確認・編集ができないため。
  Tasksビューの行とDailyの行はサムネイルのみ）。行内のリンクタップは
  行タップ（詳細シート）にせずリンク遷移、シート内のリンクタップはインライン編集を
  開始しない（画像タップは編集開始。面が画像だけでも編集の入口が残るように）。
